...
namespace Microsoft.Xna.Framework.Audio
{
    public sealed class SoundEffect : IDisposable
    {
		...
		private static List<SoundEffectInstance> _instances = new List<SoundEffectInstance>();
		...
		public SoundEffect(string fileName)
		{
			...
			_sound = new Sound(_filename);
			_name = Path.GetFileNameWithoutExtension(fileName);
		}
		
		//SoundEffect from playable audio data
		public SoundEffect(string name, byte[] data)
		{
			_data = data;
			_name = name;
			_sound = new Sound(_data);
		}        
			...
			set
			{
				if (_masterVolume != value)
				{
					...
					for (int i = 0; i < _instances.Count; i++)
					{
						...
						{
							// If the sound has been disposed, kick it out
