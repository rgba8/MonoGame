...
namespace Microsoft.Xna.Framework.Graphics
{
    ...
    public sealed class SamplerStateCollection
	{
        ...
        private SamplerState[] _previousSamplers;
        private Texture[] _previousTextures;
		...
		internal SamplerStateCollection( int maxSamplers )
        {
            ...
            _previousSamplers = new SamplerState[maxSamplers];
            _previousTextures = new Texture[maxSamplers];
            Clear();
            _dirty = true;
        }
        ...
        public void Clear()
        {
            ...
            for (var i = 0; i < _previousSamplers.Length; ++i)
            { _previousSamplers[i] = SamplerState.LinearWrap; }
            for (var i = 0; i < _previousTextures.Length; ++i)
            { _previousTextures[i] = null; }
            _dirty = true;
#if DIRECTX
        ...
        internal void SetSamplers(GraphicsDevice device)
        {
            ...
            {
                var sampler = _samplers[i];
                var previousSampler = _previousSamplers[i];
                var texture = device.Textures[i];

                if (sampler != null && texture != null)
                {
                    var previousTexture = _previousTextures[i];
                    if (_dirty || sampler.Compare(texture.glLastSamplerState))
                    {
                ...
                _previousSamplers[i] = sampler;
                _previousTextures[i] = texture;
           }
            _dirty = false;
#endif
        }
