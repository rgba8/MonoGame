            ...
            try
            {
                ...
                else
                {
                    ...
                    if (glFormat == (GLPixelFormat)All.CompressedTextureFormats)
                    {
                        if (rect.HasValue)
                        {
                            GL.CompressedTexSubImage2D(TextureTarget.Texture2D, level, x, y, w, h, glFormat, elementSizeInByte * elementCount, dataPtr);
                            GraphicsExtensions.CheckGLError();
                        }
                        else
                        {
                            GL.CompressedTexImage2D(TextureTarget.Texture2D, level, glInternalFormat, w, h, 0, elementSizeInByte * elementCount, dataPtr);
                            GraphicsExtensions.CheckGLError();
                        }
                    }
                    else
